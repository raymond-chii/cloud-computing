...
//   window.setFramerateLimit(60);
  ...
  for (int i = 0; i < num_philosopher; i++) {
    ...
    philosophers.push_back(
        Philosopher(i, chopsticks[left_index], chopsticks[right_index]));
  }

  for (int j = 0; j < num_philosopher; j++) {
    threads.push_back(
        std::thread([j, &philosophers, &simulation_done, num_philosopher]() {
  ...
  //         std::cout << "All philosophers have eaten once. Simulation
  //         complete!" << std::endl; simulation_done = true;
  //     }
  ...
  std::cout << "Total successful eat operations: " << Philosopher::total_eat
            << std::endl;
