...
Philosopher::Philosopher(int philosopher_id, Chopstick &left, Chopstick &right)
    : id(philosopher_id), current_state(Thinking), has_eaten(false),
      left_chopstick(left), right_chopstick(right) {};

void Philosopher::think() {
  current_state = Thinking;
  std::this_thread::sleep_for(
      std::chrono::milliseconds(rand() % 100)); // Shorter thinking time
...
void Philosopher::release_resource() {
  ...
  std::cout << "Philosopher " << id << " released both chopsticks."
            << std::endl;
}